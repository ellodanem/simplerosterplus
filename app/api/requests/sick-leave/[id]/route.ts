...
async function loadSickLeave(id: string, organizationId: string, locationId: string) {
  const row = await prisma.staffSickLeave.findFirst({
    where: {
      id,
      staff: { organizationId, locationId },
    },
    select: sickLeaveSelect,
  });
  if (!row) throw new RequestError("Sick leave request not found", 404);
  return row;
}
...
/**
 * PATCH /api/requests/sick-leave/[id]
...
export async function PATCH(request: Request, { params }: Ctx) {
  try {
    ...
    const row = await loadSickLeave(id, session.orgId, location.id);
    ...
    if (action === "deny") {
      const updated = await prisma.staffSickLeave.update({
      ...
      return NextResponse.json({ request: await serializeSickLeave(updated) });
    }
    ...
    const updated = await prisma.staffSickLeave.findUniqueOrThrow({
      where: { id: row.id },
      select: sickLeaveSelect,
    });
    return NextResponse.json({
      request: await serializeSickLeave(updated),
...
/**
 * DELETE /api/requests/sick-leave/[id]
...
export async function DELETE(_request: Request, { params }: Ctx) {
  try {
    ...
    return NextResponse.json({ ok: true, request: await serializeSickLeave(row) });
  } catch (e) {
    const { status, body } = errorJson(e);
    return NextResponse.json(body, { status });
  }
