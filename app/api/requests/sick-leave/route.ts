...
/**
 * POST /api/requests/sick-leave
 * Body: { staffId, startDate: YYYY-MM-DD, endDate: YYYY-MM-DD, reason? }
 *
 * Creates a `requested` sick-leave row. Approval (with conflict resolution) happens via PATCH.
 */
export async function POST(request: Request) {
  try {
    ...
    const created = await prisma.staffSickLeave.create({
      data: {
        staffId,
        startDate,
        endDate,
    ...
    return NextResponse.json(
      { request: await serializeSickLeave(created, undefined, session.orgId) },
      { status: 201 },
    );
  } catch (e) {
