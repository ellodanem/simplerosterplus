...
| Onboarding simulation (setup-write sandbox)             | ✅              | Support+ **Simulate onboarding** (Organizations list / sandbox Org 360) → resets dedicated `isOnboardingSandbox` org, mints 30 min setup-scoped session → `/setup`; emerald banner; non-setup mutations blocked; audited (`onboarding.simulate.start` / `.end`).              |
| Recording sandbox (setup → publish takes)               | ✅              | Support+ **Use for recording** on a real Clerk Free org → flags `isRecordingSandbox`. **Reset for recording** (or `npm run recording:reset`) wipes to pre-wizard; sign in normally and run setup → publish. Excluded from funnel metrics. Film signup as one-time B-roll. |
